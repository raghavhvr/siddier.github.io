/* ═══════════════════════════════════════════════
   MAIN — Init, DOM Builder, Cursor, Entry Screen
═══════════════════════════════════════════════ */
...
const DOM = {
  ...
  _buildHUDShell() {
    ...
    hud.innerHTML = `
      <div id="hud-top">
        <div id="hud-logo" onclick="Journey.scrollToChapter(0)">R · S · R</div>
        <nav id="hud-nav"></nav>
        <a href="mailto:${CONFIG.person.email}"
           style="pointer-events:all;font-size:0.58rem;letter-spacing:0.15em;
                  ...
                  padding:8px 18px;transition:background 0.3s;"
  ...
  _chapterHero() {
    ...
    <section class="chapter" id="chapter-0" data-chapter="0"
             style="min-height:100vh;--chapter-color:var(--ch1-color)">
      <div class="chapter-inner" style="padding-top:80px;">
        <div class="reveal" data-delay="0">
          ...
          <p style="font-family:var(--font-display);font-size:clamp(1.1rem,2vw,1.6rem);
                    font-style:italic;color:var(--text-secondary);max-width:560px;
  ...
  _chapterAbout() {
    ...
    const pills = ['GenAI & LLMs','ML Engineering','Data Architecture','MLOps & DevOps','Cloud Platforms','Data Governance'].map(x =>
      `<div class="expertise-pill"><span>${x}</span></div>`
    ).join('');

    return `
    <section class="chapter" id="chapter-1" data-chapter="0"
             style="--chapter-color:var(--ch1-color)">
      <div class="chapter-inner">
        <div class="grid-2">
          <div class="about-visual" style="height:420px;position:relative;">
  ...
  _chapterJourney() {
    ...
    return `
    <section class="chapter" id="chapter-2" data-chapter="1"
             style="--chapter-color:var(--ch2-color)">
      <div class="chapter-inner">
  ...
  _chapterProjects() {
    ...
    return `
    <section class="chapter" id="chapter-3" data-chapter="2"
             style="--chapter-color:var(--ch3-color)">
      <div class="chapter-inner">
    ...
    <div class="project-card reveal">
      ...
      <div class="demo-wrap">
        <div class="demo-header">
          ...
          <span class="demo-label">Live Simulator</span>
        </div>
        ...
        <div class="result-box cyan-accent">
          <div>
            ...
            <div class="result-value" id="roas-value">3.42x</div>
          </div>
          <div style="text-align:right">
            <div class="result-label">vs. baseline</div>
            <span class="result-delta" id="roas-delta">+28%</span>
          </div>
        </div>
      </div>
    </div>`;
  ...
  _demoSQL() {
    ...
    <div class="project-card reveal">
      ...
      <div class="demo-wrap">
        <div class="demo-header">
          <div class="demo-dot"></div>
          <span class="demo-label">Query Interface</span>
        </div>
  ...
  _demoCLV() {
    ...
    <div class="project-card reveal">
      ...
      <div class="demo-wrap">
        <div class="demo-header">
          <div class="demo-dot"></div>
          <span class="demo-label">Predict CLV</span>
        </div>
        <div class="input-grid">
          <div class="input-field">
            <label>Purchases (12mo)</label>
            <input type="number" id="clv-purchases" value="8" min="1" max="100">
          </div>
          <div class="input-field">
            <label>Avg Order ($)</label>
            <input type="number" id="clv-aov" value="320" min="10" max="5000">
          </div>
          <div class="input-field">
            <label>Days Since Last</label>
            <input type="number" id="clv-recency" value="14" min="1" max="365">
          </div>
          <div class="input-field">
            <label>Tenure (months)</label>
            <input type="number" id="clv-tenure" value="18" min="1" max="120">
          </div>
        </div>
        <div class="result-box gold-accent">
          <div>
            <div class="result-label">3-Year Predicted CLV</div>
            <div class="result-value gold" id="clv-value">$12,480</div>
          </div>
          <div>
            <div class="result-label">&nbsp;</div>
            <span class="result-delta" id="clv-segment" style="color:var(--cyan);">● HIGH VALUE</span>
          </div>
        </div>
      </div>
    </div>`;
  ...
  _chapterImpact() {
    ...
    return `
    <section class="chapter" id="chapter-4" data-chapter="3"
             style="--chapter-color:var(--ch4-color)">
      <div class="chapter-inner">
      ...
      <div class="chapter-inner">
        <div class="reveal">
          ...
          <div class="contact-links" style="justify-content:center;">
            ...
            <a class="contact-link" href="${p.linkedin}" target="_blank">
              <span>LinkedIn</span>
            </a>
            <a class="contact-link" href="${p.github}" target="_blank">
              <span>GitHub</span>
            </a>
          </div>
          <p style="margin-top:var(--space-xl);font-size:0.62rem;color:var(--text-dim);
